...
//   nu    - stars per solar luminosity of that component's bolometric light,
//           so the number density of bin b at x is j_c(x) * nu_c[b], with j_c
//           the component's light density from galaxyModel.js (the same
...
// It also writes the fraction of all light carried by stars fainter than
// M_V = -8 .. +12, which galaxyModel.js uses for the unresolved share.
//
...
const acc = Object.fromEntries(comps.map(c => [c, {
    n: new Float64Array(NBIN), L: 0, count: 0, lightByBin: new Float64Array(NBIN),
...
console.log(`${total} stars in ${cells.length} cells, ${((Date.now() - t0) / 1000).toFixed(1)} s; young light share ${(acc.young.L / Ltot).toFixed(3)}`);
...
// Fraction of each component's light carried by stars fainter than
// M_V = -8..+12, from the (tail-corrected) number table: light of bin b per
// unit component light = nu[b] * <L>_b, with <L>_b from the bin centre.
const faintOf = name => {
    const c = acc[name];
    const lightBin = c.nu.map((nu, b) => {
        const Mc = M_MIN + (b + 0.5) * M_STEP;
        // bolometric light of a bin-centre star, via the cell sample's mean
        // L where available (BC differs across the bin), else M_V -> L_V proxy
...
// Stars per solar luminosity (bolometric) of the component's light, per bin.
...
// Fraction of each component's bolometric light carried by stars fainter
// than M_V = -8, -7, ..., +12, and of the local mixture.
...
// Young share of the local light in the sampled cells (galaxyModel MW.fYoung).
