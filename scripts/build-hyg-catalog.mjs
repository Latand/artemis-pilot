...
for (const line of lines) {
    ...
    // AT-HYG's own "hyg" column is this HYG database's "id" field verbatim (verified
    // against Sirius/Vega); "hip" is the Hipparcos ID. Both are collected here, for every
    // row actually retained in the Tier-0 binary, so scripts/build-athyg-tier1.mjs can
    // exclude the same physical stars from the Tier-1 AT-HYG tiles.
    const tier0Id = num(row[IDX.id]);
    if (tier0Id !== null) tier0HygIds.push(tier0Id);
    const tier0Hip = num(row[IDX.hip]);
    if (tier0Hip !== null) tier0HipIds.push(tier0Hip);
...
const OUT_TIER0_IDS = new URL("../public/data/hyg-tier0-ids.json", import.meta.url);
await writeFile(OUT_TIER0_IDS, JSON.stringify({
    schema: 1,
    source: "Derived from this HYG v4.1 build: the database's own 'id' field and the Hipparcos 'hip' field, for every row retained in hyg-stars-v41.bin",
    usage: "scripts/build-athyg-tier1.mjs excludes AT-HYG rows whose hyg/hip id appears here, so Tier-1 never duplicates a Tier-0 star",
    hyg: uniqSorted(tier0HygIds),
    hip: uniqSorted(tier0HipIds),
}));

console.log(`wrote ${values.length / 10} HYG stars, ${labels.length} labels, ${stats.massEstimated} mass estimates, ${physicalStars.length} physical runtime stars, ${OUT.pathname}, ${OUT_BIN.pathname}, ${OUT_PHYSICAL.pathname}, and ${OUT_TIER0_IDS.pathname}`);
