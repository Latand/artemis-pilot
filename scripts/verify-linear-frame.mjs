...
function frame(n,hdr){scene.clear();for(let i=0;i<n;i++){
 const m=new THREE.ShaderMaterial({uniforms:{light:{value:.9/n}},transparent:true,depthTest:false,depthWrite:false,blending:THREE.AdditiveBlending,
...
renderLinearFrame(renderer,()=>renderer.render(scene,camera));
const gl=renderer.getContext(),red=new Uint8Array(4);gl.readPixels(64,48,1,1,gl.RGBA,gl.UNSIGNED_BYTE,red);
...
window.result={single,encodedOverlap,linearOverlap,foreground:[...red],restored,threw,noNested,stats:{...linearFrame},renderer:gl.getParameter(gl.VERSION)};
</script>`;
...
assert(r.foreground[0]>100&&r.foreground[1]===0&&r.foreground[2]===0,'Opaque foreground occludes the linear background');
